...
export function enumBitSetFromNumber(
  bitset: number,
  allValues: any
): ReadonlySet<number> {
...
export function parseQueryStatusRegisterResponse(
  response: Uint8Array
): {
  pnow: number;
  almc: number;
  ...
  const dss1 = (enumBitSetFromNumber(
    view.getUint16(10),
    DSS1
  ) as any) as Set<DSS1>;
  const dss2 = (enumBitSetFromNumber(
    view.getUint16(12),
    DSS2
  ) as any) as Set<DSS2>;
  const dsse = (enumBitSetFromNumber(
    view.getUint16(14),
    DSSE
  ) as any) as Set<DSSE>;
  const stat = (enumBitSetFromNumber(
    view.getUint32(16),
    STAT
  ) as any) as Set<STAT>;
 ...
 */
export const resetAlarm: Readonly<[ArrayBuffer, ArrayBuffer]> = [
  buildResetAlarm(true),
  buildResetAlarm(false),
];
...
export function positionCommand(targetPosition: number): ArrayBuffer {
  ...
  v.setUint16(11, crc16modbus(new Uint8Array(buffer, 0, 11) as any), true);
  return buffer;
}
...
function buildHomeReturn(ret: boolean): ArrayBuffer {
  ...
  d1.setUint16(6, crc16modbus(new Uint8Array(buffer, 0, 6) as any), true);
  return buffer;
}
 ...
 */
export const homeReturn: Readonly<[ArrayBuffer, ArrayBuffer]> = [
  buildHomeReturn(false),
  buildHomeReturn(true),
];

export function positionVelocityAndAccelerationCommand(
  ...
  v.setUint16(21, crc16modbus(new Uint8Array(buffer, 0, 21) as any), true);
  return buffer;
}
...
export function numericalValueMovementCommand(
  ...
  view.setUint16(25, crc16modbus(new Uint8Array(buffer, 0, 25) as any), true);
  return buffer;
}
...
export function parseException(
  ...
  if ((functionCode & 0x80) !== 0x80) {
    throw new Error(
      "Response is not an exception (Function Code MSB is not set to 1)."
    );
  }
  const originalFunctionCode = functionCode ^ 0x80;
  const exceptionCode = view.getUint8(2);
  const exceptionMessage =
    exceptionMap[exceptionCode - 1] || "Unknown Exception Code";
...
export function parseDeviceMovementHistory(
  response: Uint8Array
): { totalMovingCount: number; totalMovingDistance: number } {
  const data = parseQueryHoldingRegistersResponse(response);
  const view = new DataView(data);
  return {
    totalMovingCount: view.getUint32(0),
    totalMovingDistance: view.getUint32(4),
  };
}
...
export function parseInputSignalStatusResponse(
  response: Uint8Array
): { inputStatus: number } {
  crcCheck(response);
...
function forceSingleCoil(address: number, data: number): ArrayBuffer {
  ...
  view.setUint16(6, crc16modbus(new Uint8Array(buffer, 0, 6) as any), true);
  return buffer;
}
...
function parseQueryHoldingRegistersResponse(
  response: Uint8Array
): ArrayBufferLike {
  crcCheck(response);
  const view = new DataView(response.buffer);
...
export function parseAlarmDetailDescriptionReadingResponse(
  response: Uint8Array
) {
  const data = parseQueryHoldingRegistersResponse(response);
...
function crcCheck(response: Uint8Array): void {
  ...
  const expectedCrc = crc16modbus(
    new Uint8Array(response.buffer, 0, response.length - 2) as any
  );
  const presentedCrc = dataView.getUint16(response.length - 2, true);
  if (expectedCrc !== presentedCrc) {
    throw new Error(
      `CRC missmatch: Expected ${presentedCrc} but received ${expectedCrc}`
    );
  }
}
